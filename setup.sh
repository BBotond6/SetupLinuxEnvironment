...
if command -v apt; then
	...
	if command -v python3; then
		echo -e "${YELLOW}Step 2:${END_COLOR} Install Python3 | ${GREEN}DONE${END_COLOR} \n note: Python3 is already installed!"
	else
		echo -e "${YELLOW}Step 2:${END_COLOR} Install Python3 \nPython3 is not installed! Installing..."
		sudo apt install python3
		if command -v python3; then
			echo -e "${YELLOW}Step 2:${END_COLOR} Install Python3 | ${GREEN}DONE${END_COLOR}"
		else
			echo -e "${YELLOW}Step 2:${END_COLOR} Install Python3 | ${RED}FAILED${END_COLOR} \n note: Installation of Python3 was unsuccessful!"
			
		fi
	fi
	
	if [ -e "$setup_py" ]; then
		echo -e "${YELLOW}Step 3:${END_COLOR} Find and run $setup_py | ${GREEN}DONE${END_COLOR} \n\nRun $setup_py for further configurations...\n"
		python3 setup.py
	else
		echo -e "${YELLOW}Step 3:${END_COLOR} Find and run $setup_py | ${RED}FAILED${END_COLOR} \n note: $setup_py does not exist!"
		exit 0
	fi
